 ...
 * method.
 * 
 * Copyright (c) 2015 by WCMC
 */

define('PUBMEDAPI_VERSION', '1'); // 05 May 2015
...
class PubMedAPI
{
	...
	//private $esearch = 'http://eutils.ncbi.nlm.nih.gov/entrez/eutils/egquery.fcgi?';	
	private $efetch = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?';
	
	private $base = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?';
	...
	public function query($term, $compact = false, $callback = false)
	{
		...
		//$xml = $this->pubmed_efetch($this->term);
	...
	protected function pubmed_esearch($term)
	{
		...
		$params = array(
			...
			'term' => str_replace('%2B','+', stripslashes(urlencode($term)))
		);
		foreach ($params as $key => $value) { $q[] = $key . '=' . $value; }
		//$term=str_replace('%255D',']',str_replace('%255B','[',str_replace('%2529',')',str_replace('%2528','(',str_replace('%2B','+', stripslashes(urlencode($term)))))));
		//$url = $base . "db=".$this->$db."&term=".$term."&usehistory=y";
		
		$httpquery = implode('&',$q);
		$httpquery=str_replace("++","+",$httpquery);
		$httpquery=str_replace("%2522+","",$httpquery);
		$httpquery=str_replace("%2522","",$httpquery);
		//echo $httpquery."<br />";
